from django.shortcuts import render
from rest_framework.views import APIView
from rest_framework.response import Response
from rest_framework import authentication, permissions
from django.contrib.auth.models import User
from django.http import HttpResponse
from rest_framework.decorators import api_view,schema
from rest_framework.response import Response
from rest_framework import status
from rest_framework import mixins
from rest_framework import generics
from django_filters.rest_framework import DjangoFilterBackend
from rest_framework.filters import SearchFilter, OrderingFilter
from rest_framework.renderers import JSONRenderer
from rest_framework.parsers import JSONParser
from rest_framework.parsers import MultiPartParser
from rest_framework.pagination import PageNumberPagination
from django.shortcuts import get_object_or_404
from django.contrib.auth.models import Group,Permission
from django.contrib.auth.hashers import make_password
from .models import *
from .serializers import *
from .filters import *

# Create your views here.
#新增
class UserCreateView(generics.CreateAPIView):
    '''
        1 添加用户
        ---
        #### 参数说明
        |字段名称|描述|必须|类型|
        |--|--|--|--|
        |User_addtime|用户添加时间|False|datatime|
        |User_name|用户名称|False|string|
        |User_detail|用户描述|False|string|
        |User_address|用户地址|False|string|
        |User_user|用户名|False|string|
        |User_passwd|密码|False|string|

        #### 响应字段说明
        |字段名称|描述|必须|类型|
        |--|--|--|--|
        |id|用户id|--|int|
        |User_addtime|用户添加时间|False|datatime|
        |User_name|用户名称|False|string|
        |User_detail|用户描述|False|string|
        |User_address|用户地址|False|string|
        |User_user|用户名|False|string|
        |User_passwd|密码|False|string|

        ### 响应消息：
        |Http响应码|原因|响应模型|
        |--|--|--|
        |201|添加成功|返回用户信息|
    '''
    module_perms = ['users.user_contro']
    serializer_class = UserSerializer

    # 自定义返回结果
    def create(self, request, *args, **kwargs):
        data= request.POST.copy() # 这个QueryDict实例是不可变的
        data['password']=make_password(data['password'])
        serializer = self.get_serializer(data=data)
        try:  # 失败信息的定制
            serializer.is_valid(raise_exception=True)
        except:
            return Response(data={'code': 400, 'message': '添加用户失败'}, status=status.HTTP_400_BAD_REQUEST)
        self.perform_create(serializer)
        headers = self.get_success_headers(serializer.data)
        # 成功后返回信息的定制
        res = Response(serializer.data, status=status.HTTP_201_CREATED, headers=headers)
        res.data['code'] = 201
        res.data['message'] = "亲，添加成功了哦"
        return res

#用户列表
class UserListView(generics.ListAPIView):
    '''
        2 用户列表
        ---
        ### 参数说明
        |字段名称|描述|必须|类型|
        |--|--|--|--|
        |User_addtime|用户添加时间|False|datatime|
        |User_name|用户名称|False|string|
        |User_detail|用户描述|False|string|
        |User_address|用户地址|False|string|
        |User_user|用户名|False|string|
        |User_passwd|密码|False|string|

        ### 响应字段说明
        |字段名称|描述|必须|类型|
        |--|--|--|--|
        |id|用户id|--|int|
        |User_addtime|用户添加时间|False|datatime|
        |User_name|用户名称|False|string|
        |User_detail|用户描述|False|string|
        |User_address|用户地址|False|string|
        |User_user|用户名|False|string|
        |User_passwd|密码|False|string|

        ### 响应消息：
        |Http响应码|原因|响应模型|
        |--|--|--|
        |200|请求成功|返回用户信息|
    '''
    module_perms = ['users.user_view']
    queryset = Users.objects.all()
    serializer_class = UserListSerializer
    # pagination_class = GoodsPagination  # 自定义分页器

    # 过滤器使用
    # (DjangoFilterBackend过滤,SearchFilter搜索,OrderingFilter排序）
    filter_backends = (DjangoFilterBackend, SearchFilter, OrderingFilter)
    # filter_fields = ('')
    search_fields = ('username','last_name','user_number','user_tel','user_gender','user_detail')  # 模糊查询- 搜索
    ordering_fields = ('-id')


class UserDeleteView(generics.GenericAPIView):
    '''
        3 删除用户
        ---
        ### 参数说明
        |字段名称|描述|必须|类型|
        |--|--|--|--|
        |id|用户id|Ture|int|

        ### 响应字段说明
        |字段名称|描述|必须|类型|
        |--|--|--|--|
        |id|用户id|--|int|
        |User_addtime|用户添加时间|False|datatime|
        |User_name|用户名称|False|string|
        |User_detail|用户描述|False|string|
        |User_address|用户地址|False|string|
        |User_user|用户名|False|string|
        |User_passwd|密码|False|string|

        ### 响应消息：
        |Http响应码|原因|响应模型|
        |--|--|--|
        |200|请求成功|返回用户信息|
    '''
    module_perms = ['users.user_contro']
    queryset =  Users.objects.all()
    serializer_class = UserDeleteSerializer

    def get(self,request,pk):
        #获取地址url中的参数
        user_id = self.kwargs.get('pk','')
        try:
            obj = Users.objects.get(pk=pk)
            obj.delete()
        except:
            return Response(data={'code': 400, 'message': '亲，删除失败'}, status=status.HTTP_400_BAD_REQUEST)
        return Response(data={'code':200,'message':'亲，删除成功'},status=status.HTTP_200_OK)

#更改用户信息 post
class UserUpdateView(generics.GenericAPIView,mixins.UpdateModelMixin):
    '''
        4 修改用户信息
        ---
        ### 参数说明
        |字段名称|描述|必须|类型|
        |--|--|--|--|
        |User_addtime|用户添加时间|False|datatime|
        |User_name|用户名称|False|string|
        |User_detail|用户描述|False|string|
        |User_address|用户地址|False|string|
        |User_user|用户名|False|string|
        |User_passwd|密码|False|string|

        ### 响应字段说明
        |字段名称|描述|必须|类型|
        |--|--|--|--|
        |id|用户id|--|int|
        |User_addtime|用户添加时间|False|datatime|
        |User_name|用户名称|False|string|
        |User_detail|用户描述|False|string|
        |User_address|用户地址|False|string|
        |User_user|用户名|False|string|
        |User_passwd|密码|False|string|

        ### 响应消息：
        |Http响应码|原因|响应模型|
        |--|--|--|
        |200|请求成功|返回用户信息|
    '''
    module_perms = ['users.user_contro']
    queryset = Users.objects.all()
    serializer_class = UserInfoUpdateSerializer
    #数据编辑用post方式
    def post(self,request,*args,**kwargs):
        try:
            def update(self, request, *args, **kwargs):
                partial = kwargs.pop('partial', False)
                instance = self.get_object()
                data = request.POST.copy()  # 这个QueryDict实例是不可变的
                data['password'] = make_password(data['password'])
                serializer = self.get_serializer(instance, data=request.data, partial=partial)
                serializer.is_valid(raise_exception=True)
                self.perform_update(serializer)

                if getattr(instance, '_prefetched_objects_cache', None):
                    # If 'prefetch_related' has been applied to a queryset, we need to
                    # forcibly invalidate the prefetch cache on the instance.
                    instance._prefetched_objects_cache = {}

                return Response(serializer.data)
            update(request, *args, **kwargs)
        except:
            return Response(data={'code':400,'message':'修改失败',},status=status.HTTP_400_BAD_REQUEST)
        return Response(data={'code':200,'message':'修改成功'},status=status.HTTP_200_OK)

#更改用户状态 post
class UserStatusUpdateView(generics.GenericAPIView,mixins.UpdateModelMixin):
    '''
        5 修改用户状态
        ---
        ### 参数说明
        |字段名称|描述|必须|类型|
        |--|--|--|--|
        |User_addtime|用户添加时间|False|datatime|
        |User_name|用户名称|False|string|

        ### 响应字段说明
        |字段名称|描述|必须|类型|
        |--|--|--|--|
        |id|用户id|--|int|
        |User_addtime|用户添加时间|False|datatime|
        |User_name|用户名称|False|string|
        |User_detail|用户描述|False|string|
        |User_address|用户地址|False|string|
        |User_user|用户名|False|string|
        |User_passwd|密码|False|string|

        ### 响应消息：
        |Http响应码|原因|响应模型|
        |--|--|--|
        |200|请求成功|返回用户信息|
    '''
    module_perms = ['users.user_contro']
    queryset = Users.objects.all()
    serializer_class = UserUpdateSerializer
    #数据编辑用post方式
    def post(self,request,*args,**kwargs):
        # print('request:',request)
        # print(kwargs)
        try:
            self.update(request,*args,**kwargs)  #调用UpdateModelMixin方法
        except:
            return Response(data={'code':400,'message':'修改失败',},status=status.HTTP_400_BAD_REQUEST)
        return Response(data={'code':200,'message':'修改成功'},status=status.HTTP_200_OK)

